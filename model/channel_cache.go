...
func ListSatisfiedChannels(group string, model string, requestPath string) ([]*Channel, error) {
	if !common.MemoryCacheEnabled {
		return listSatisfiedChannelsDB(group, model, requestPath)
	}
...
func listSatisfiedChannelsDB(group string, model string, requestPath string) ([]*Channel, error) {
	channelIDs, err := listSatisfiedChannelIDsDB(group, model)
	if err != nil {
		return nil, err
	}
	if len(channelIDs) > 0 {
		channels, err := loadChannelsByOrderedIDs(channelIDs)
		if err != nil {
			return nil, err
		}
