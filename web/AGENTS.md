...
- 修改语言转换或数字格式化时，遵循根目录测试规则，使用已有测试及人工检查验证 `zhCN`、`zhTW`、其余五种界面语言，以及非法语言的降级；涉及组件语言响应时还须验证切换语言。修改 lint 规则须验证违规代码报错且合法转换不误报，类型检查和构建通过不能替代这些验证，不得为此新增或改写测试。
...
- 遵循根目录 `AGENTS.md` 的测试规则：保留上游原有测试，不得新增、修改、删除、重命名或重组测试文件和用例；测试快照、fixture 与 helper 同样保留原样。
- 新增功能、修复缺陷、调整 UI 行为或重构均不得补充或改写测试，不设置新增覆盖率要求，也不要求先编写失败用例。
- 按改动范围按需运行已有且不依赖数据库的上游测试；不要求运行数据库测试，不因未执行数据库测试阻塞完成。
- 已有测试失败时如实记录原因和影响；不得通过调整断言、更新快照、跳过用例或更改测试脚本、CI 配置掩盖失败。尚未验证的行为需如实说明。
- 继续遵守类型检查、lint、格式检查和构建要求；只报告实际执行的检查及结果，未执行的测试不得声称通过。
