...
it('keeps full mobile information without group or quota section headings', async () => {
  ...
  try {
    await renderKeysPage()
    const card = screen.getByText(key.name).closest<HTMLElement>('.bg-card')
    if (!card) throw new Error('API key mobile card not found')
    const mobileCard = within(card)

    expect(screen.queryByRole('table')).not.toBeInTheDocument()
    expect(mobileCard.queryByText('额度 ($)')).not.toBeInTheDocument()
    expect(mobileCard.getByText('($)')).toBeInTheDocument()
    expect(
      mobileCard.getByRole('button', { name: /剩余 80;/ })
    ).toBeInTheDocument()
    expect(mobileCard.getByText('80')).toBeInTheDocument()
    expect(mobileCard.getByText('120')).toBeInTheDocument()
    expect(mobileCard.getByText(zh.translation['Created'])).toBeInTheDocument()
    expect(
      mobileCard.getByText(zh.translation['Last Used'])
    ).toBeInTheDocument()
    expect(mobileCard.getByText(zh.translation['Expires'])).toBeInTheDocument()
    expect(
      mobileCard.queryByText(zh.translation['Group'], { exact: true })
    ).not.toBeInTheDocument()
    expect(mobileCard.getByText('default')).toBeInTheDocument()
    expect(mobileCard.getByText('1x')).toBeInTheDocument()
    expect(mobileCard.getByText(zh.translation['Models'])).toBeInTheDocument()
    expect(
      mobileCard.getByText(zh.translation['IP Restriction'])
    ).toBeInTheDocument()
  } finally {
