...
Only what was actually run. Each item needs the command or steps and the observed
result; do not state only that `go build` or tests passed.
Preserve existing upstream tests without adding,
